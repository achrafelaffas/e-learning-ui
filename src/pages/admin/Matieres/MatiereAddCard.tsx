...
function MatiereAddCard({ setMatieres }: MatiereProps) {
    ...
    return (
        <>
            <header className="p-4 lg:flex lg:gap-x-12">
                <h1 className="text-2xl font-bold">Matières</h1>
                <div className="lg:flex lg:flex-1 lg:justify-end">
                    <Button onClick={() => setOpen(true)}>Ajouter une nouvelle matière</Button>
                </div>
            </header>
...
function MatiereForm({
    ...
    const handleSaveClick = async (e: SyntheticEvent) => {
        ...
        if (selectedFile && newMatiere) {
            try {
                const reponse = await matiereApi.createMatiereWithImage(matiere, selectedFile);
